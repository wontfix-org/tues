...
def provider(provider, args):
    ...
    try:
        ...
        raise TuesLookupError(f"Error running provider: {_shlex.join(cmd)}") from e
...
class Script:
    ...
    def __init__(self, cmd, paths=None):
        ...
        if isinstance(cmd, str):
            self.script, *self.args = _shlex.split(cmd)
        else:
            self.script, *self.args = cmd
    ...
    def run(
        ...
        **kwargs,
        ...
        run_kwargs.setdefault("files", []).append(self.path)
        ...
        return run(cmd=["./" + _os.path.basename(self.path)] + self.args, **run_kwargs)
...
async def _prepare_io(run, stdout, stderr, env, pm, send_input):
    ...
    def _pick(f):
        ...
        if run.capture_output or f is PIPE:
            buf = BufferedIO(mode="wt+" if run.text is True else "wb+", encoding=run.encoding, errors=run.errors)
